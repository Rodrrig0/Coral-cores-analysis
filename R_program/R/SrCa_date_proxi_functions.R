...
peak_to_date <- function(data, top_date, min_sst_month, max_sst_month) {
  data$date[1] <- decimal_date(as.POSIXct(strptime(top_date, "%d/%m/%Y")))
  ...
  if (target_max_date > data$date[1]) {
    target_max_date <- target_max_date - 1  #remove a year
  }


  if (target_min_date > data$date[1]) {
    target_min_date <- target_min_date - 1  #remove a year
  }
